...
let _temp = {
	...
	formatMilliseconds: (ms) => {
		...
		return Object.entries(time).filter((val) => val[1] !== 0).map(([
			key, val
	...
	copy: (text, mimeType = "text/html") => {
		try {
			...
			console.warn(err,
				"Your browser does not support the navigator.clipboard API. Falling back to copying plain text instead."
				);
			el.value = text;
	...
	browser: () => {
		node();
		var isOpera = (!!window.opr && !!opr.addons) || !!window
		...
		var isSafari = /constructor/i.test(window.HTMLElement) || (
			...
			"undefined" && window["safari"].pushNotification));
		var isIE = /*@cc_on!@*/ false || !!document.documentMode;
		var isEdge = !isIE && !!window.StyleMedia;
		var isChrome = !!window.chrome && (!!window.chrome.webstore || !
			!window.chrome.runtime);
		var isEdgeChromium = isChrome && navigator.userAgent.indexOf(
			"Edg") != -1;
	 ...
	 */
	dayName: (date, locale = "en-US") => date.toLocaleDateString(locale, {
		weekday: "long",
	}),
	/**
	...
	jsonToCsv: (arr, columns, delimiter = ",") => [
		columns.join(delimiter), ...arr.map((obj) => columns.reduce(
			(acc, key) => `${acc}${!acc.length ? "" : delimiter}"${
              !obj[key] ? "" : obj[key]
            }"`, "")),
	].join("\n"),
	/**
	 ...
	 */
	mapObjectKeys: (obj, fn) => Array.isArray(obj) ? obj.map((val) => _$
			.mapObjectKeys(val, fn)) : typeof obj === "object" ? Object
		.keys(obj).reduce((acc, current) => {
			const key = fn(current);
			const val = obj[current];
			acc[key] = val !== null && typeof val === "object" ? _$
				.mapObjectKeys(val, fn) : val;
			return acc;
		}, {}) : obj,
	/**
	 ...
	 */
	arrayToCSV: (arr, delimiter = ",") => arr.map((v) => v.map((x) => (
		isNaN(x) ? `"${x.replace(/"/g, '""')}"` : x)).join(
		delimiter)).join("\n"),
	/**
	 * averageBy
	 * @function
	 ...
	 */
	averageBy: (arr, fn) => arr.map(typeof fn === "function" ? fn : (val) =>
		val[fn]).reduce((acc, val) => acc + val, 0) / arr.length,
	/**
	...
	inView: (el) => {
		...
		return (top >= window.pageYOffset && left >= window
			.pageXOffset && top + height <= window.pageYOffset +
			window.innerHeight && left + width <= window
	...
	inPartialView: (el) => {
		...
		return (top < window.pageYOffset + window.innerHeight && left <
			window.pageXOffset + window.innerWidth && top + height >
			window.pageYOffset && left + width > window.pageXOffset);
	},
	/**
	...
	serializeForm: (form) => {
		node();
		return Array.from(new FormData(form), (field) => field.map(
			encodeURIComponent).join("=")).join("&");
	},
	/**
	...
	formToObject: (form) => {
		node();
		return Array.from(new FormData(form)).reduce((acc, [key,
			value]) => ({
				...acc,
				[key]: value,
			}));
	...
	replaceText: (el, callback) => {
		...
		for (var e,
				t = (function() {
					for (var e, t = el, o = [], a = 0; a < t
						.length; a++)
						(e = t[a].childNodes[0]),
						t[a].hasChildNodes() && 3 == e.nodeType && o
						.push(e);
					return o;
				})(),
				o = 0,
	...
	*/
	lightOrDark: (color) => {
		...
		if (color.match(/^rgb/)) {
			...
			color = +("0x" + color.slice(1).replace(color.length < 5 &&
				/./g, "$&$&"));
			r = color >> 16;
			g = (color >> 8) & 255;
			b = color & 255;
		}
		hsp = Math.sqrt(0.299 * (r * r) + 0.587 * (g * g) + 0.114 * (b *
			b));
		if (hsp > 127.5) {
			return {
				lightOrDark: "light",
	...
	hexToRGB: (hex) => {
		...
		h = parseInt(h, 16);
		return ("rgb" + (alpha ? "a" : "") + "(" + (h >>> (alpha ? 24 :
			16)) + ", " + ((h & (alpha ? 0x00ff0000 :
			0x00ff00)) >>> (alpha ? 16 : 8)) + ", " + ((h & (
			alpha ? 0x0000ff00 : 0x0000ff)) >>> (alpha ? 8 :
			0)) + (alpha ? `, ${h & 0x000000ff}` : "") + ")");
	},
	/**
	...
	querySelector: (elem) => {
		...
		function loop(element) {
			if (element.getAttribute("id") && document.querySelectorAll(
					`#${element.getAttribute("id")}`).length === 1) {
				str = str.replace(/^/, " #" + element.getAttribute(
					"id"));
				str = str.replace(/\s/, "");
				str = str.replace(/\s/g, " > ");
				return str;
			...
			if (element.getAttribute("class")) {
				...
				for (var i = 0; i < childrens.length; i++) {
					if (element.getAttribute("class") == childrens[i]
						.getAttribute("class")) {
	...
	*/
	 ...
	 */
	formatNumber: (n) => n.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g,
		"$1,"),
	/**
	...
	spliceArrayBuffer: (arr, start, end, endian) => {
		...
		for (var i = start, value = 0; i != end; i += direction) value =
			256 * value + arr[i];
		return value;
	},
	/**
	...
	unCamelCase: function(str) {
		return str.replace(/([a-z])([A-Z])/g, "$1 $2").replace(
			/\b([A-Z]+)([A-Z])([a-z])/, "$1 $2$3").replace(/^./,
			function(s) {
	...
	syntaxHighlight: (string, mode = "html", colors = {}) => {
		...
		let highlightel = (elmnt, mode, colors = {}) => {
			...
			var attributevaluecolor = colors.attributeValueColor ||
				"mediumblue";
			var commentcolor = colors.commentColor || "green";
			var cssselectorcolor = colors.cssSelectorColor ||
				"brown";
			var csspropertycolor = colors.cssPropertyColor || "red";
			var csspropertyvaluecolor = colors
				.cssPropertyValueColor || "mediumblue";
			var cssdelimitercolor = colors.cssLimiterColor ||
				"black";
			var cssimportantcolor = colors.cssImportantColor ||
				"red";
			var jscolor = colors.jsColor || "black";
			var jskeywordcolor = colors.jsKeywordColor ||
				"mediumblue";
			...
			elmntObj.style.fontFamily = colors.fontFamily ||
			...
			function extract(str, start, end, func, repl) {
				...
				while (str.search(start) > -1) {
					...
					if (repl) {
						a.push(func(str.substring(s, e + end
							.length)));
						str = str.substring(0, s) + repl + str
							.substr(e + end.length);
					} else {
						d += str.substring(0, s);
			...
			function htmlMode(txt) {
				...
				comment = new extract(rest, "&lt;!--", "--&gt;",
					commentMode, "W3HTMLCOMMENTPOS");
				rest = comment.rest;
				while (rest.indexOf("&lt;") > -1) {
					note = "";
					startpos = rest.indexOf("&lt;");
					if (rest.substr(startpos, 9).toUpperCase() ==
						"&LT;STYLE") {
						note = "css";
					}
					if (rest.substr(startpos, 10).toUpperCase() ==
						"&LT;SCRIPT") {
						note = "javascript";
					}
					...
					done += tagMode(rest.substring(startpos,
						endpos + 4));
					rest = rest.substr(endpos + 4);
					if (note == "css") {
						endpos = rest.indexOf("&lt;/style&gt;");
						if (endpos > -1) {
							done += cssMode(rest.substring(0,
								endpos));
				...
				for (i = 0; i < comment.arr.length; i++) {
					rest = rest.replace("W3HTMLCOMMENTPOS", comment
						.arr[i]);
				}
				return rest;
			}
			...
			function tagMode(txt) {
				...
				while (rest.search(/(\s|<br>)/) > -1) {
					...
					done += attributeMode(rest.substring(startpos,
						endpos));
					rest = rest.substr(endpos);
				}
				result = done + rest;
				result = "<span style=color:" + tagcolor +
					">&lt;</span>" + result.substring(4);
				if (result.substr(result.length - 4, 4) == "&gt;") {
					result = result.substring(0, result.length -
						4) + "<span style=color:" + tagcolor +
						">&gt;</span>";
				}
				return "<span style=color:" + tagnamecolor + ">" +
					result + "</span>";
			}
			...
			function attributeMode(txt) {
				...
				while (rest.indexOf("=") > -1) {
					...
					if (spacepos > -1 && (spacepos <
							singlefnuttpos || singlefnuttpos == -1
						...
						endpos = rest.indexOf(" ", startpos);
							...
							singlefnuttpos == -1) && (
							doublefnuttpos < spacepos || spacepos ==
							-1)) {
						endpos = rest.indexOf('"', rest.indexOf('"',
							startpos) + 1);
					} else if (singlefnuttpos > -1 && (
							singlefnuttpos < doublefnuttpos ||
							doublefnuttpos == -1) && (
							singlefnuttpos < spacepos || spacepos ==
							-1)) {
						endpos = rest.indexOf("'", rest.indexOf("'",
							startpos) + 1);
					}
					if (!endpos || endpos == -1 || endpos <
						startpos) {
						endpos = rest.length;
					}
					done += rest.substring(0, startpos);
					done += attributeValueMode(rest.substring(
						startpos, endpos + 1));
					rest = rest.substr(endpos + 1);
				}
				return ("<span style=color:" + attributecolor +
					">" + done + rest + "</span>");
			}

			function attributeValueMode(txt) {
				return ("<span style=color:" + attributevaluecolor +
					">" + txt + "</span>");
			}

			function commentMode(txt) {
				return "<span style=color:" + commentcolor + ">" +
					txt + "</span>";
			}
			...
			function cssMode(txt) {
				...
				while (rest.search("{") > -1) {
					...
					done += cssPropertyMode(rest.substring(s + 1,
						e));
					rest = rest.substr(e);
				}
				rest = done + rest;
				rest = rest.replace(/{/g, "<span style=color:" +
					cssdelimitercolor + ">{</span>");
				rest = rest.replace(/}/g, "<span style=color:" +
					cssdelimitercolor + ">}</span>");
				for (i = 0; i < comment.arr.length; i++) {
					rest = rest.replace("W3CSSCOMMENTPOS", comment
						.arr[i]);
				}
				return "<span style=color:" + cssselectorcolor +
					">" + rest + "</span>";
			}
			...
			function cssPropertyMode(txt) {
				...
				while (rest.search(":") > -1) {
					...
					done += cssPropertyValueMode(rest.substring(s,
						e + 1));
					rest = rest.substr(e + 1);
				}
				return ("<span style=color:" + csspropertycolor +
					">" + done + rest + "</span>");
			}
			...
			function cssPropertyValueMode(txt) {
				var rest = txt,
					...
					s;
				rest = "<span style=color:" + cssdelimitercolor +
					">:</span>" + rest.substring(1);
				while (rest.search(/!important/i) > -1) {
					s = rest.search(/!important/i);
					done += rest.substring(0, s);
					done += cssImportantMode(rest.substring(s, s +
						10));
				...
				if (result.substr(result.length - 1, 1) == ";" &&
					...
					4) != "&lt;" && result.substr(result.length - 4,
						4) != "&gt;" && result.substr(result
						.length - 5, 5) != "&amp;") {
					result = result.substring(0, result.length -
						1) + "<span style=color:" +
						cssdelimitercolor + ">;</span>";
				...
				return ("<span style=color:" +
					csspropertyvaluecolor + ">" + result +
					"</span>");
			}

			function cssImportantMode(txt) {
				return ("<span style=color:" + cssimportantcolor +
			...
			function jsMode(txt) {
				...
				y = 1;
				while (y == 1) {
					sfnuttpos = getPos(rest, "'", "'",
					jsStringMode);
					dfnuttpos = getPos(rest, '"', '"',
					jsStringMode);
					compos = getPos(rest, /\/\*/, "*/",
					commentMode);
					comlinepos = getPos(rest, /\/\//, "<br>",
						commentMode);
					numpos = getNumPos(rest, jsNumberMode);
					keywordpos = getKeywordPos("js", rest,
						jsKeywordMode);
					dotpos = getDotPos(rest, jsPropertyMode);
					if (Math.max(numpos[0], sfnuttpos[0], dfnuttpos[
								0], compos[0], comlinepos[0],
							keywordpos[0], dotpos[0]) == -1) {
						break;
					}
					mypos = getMinPos(numpos, sfnuttpos, dfnuttpos,
						compos, comlinepos, keywordpos, dotpos);
					if (mypos[0] == -1) {
						break;
					}
					if (mypos[0] > -1) {
						done += rest.substring(0, mypos[0]);
						done += mypos[2](rest.substring(mypos[0],
							mypos[1]));
				...
				return "<span style=color:" + jscolor + ">" + rest +
					"</span>";
			}

			function jsStringMode(txt) {
				return "<span style=color:" + jsstringcolor + ">" +
					txt + "</span>";
			}

			function jsKeywordMode(txt) {
				return "<span style=color:" + jskeywordcolor + ">" +
					txt + "</span>";
			}

			function jsNumberMode(txt) {
				return "<span style=color:" + jsnumbercolor + ">" +
					txt + "</span>";
			}

			function jsPropertyMode(txt) {
				return "<span style=color:" + jspropertycolor +
					">" + txt + "</span>";
			}
			...
			function getKeywordPos(typ, txt, func) {
				var words,
					...
					patt;
				if (typ == "js") {
					words = ["abstract", "arguments", "boolean",
				...
				for (i = 0; i < words.length; i++) {
					...
					if (pos > -1) {
						patt = /\W/g;
						if (txt.substr(pos + words[i].length, 1)
							.match(patt) && txt.substr(pos - 1, 1)
							.match(patt)) {
							if (pos > -1 && (rpos == -1 || pos <
									rpos)) {
								rpos = pos;
	 ...
	 */
	composeFunction: (...functions) => (args) => functions.reduceRight((arg,
		fn) => fn(arg), args),
	/**
	 ...
	 */
	curryFunction: (fn, arity = fn.length, ...args) => arity <= args
		.length ? fn(...args) : curry.bind(null, fn, arity, ...args),
	/**
	...
	drag: (el) => {
		...
		function repositionElement(event) {
			this.style.left = initX + event.clientX - mousePressX +
			"px";
	...
	ease: {
		...
		// acceleration until halfway, then deceleration
		easeInOutCubic: (t) => t < 0.5 ? 4 * t * t * t : (t - 1) * (2 * t -
			2) * (2 * t - 2) + 1,
		// accelerating from zero velocity
		...
		// acceleration until halfway, then deceleration
		easeInOutQuart: (t) => t < 0.5 ? 8 * t * t * t * t : 1 - 8 * --t *
			t * t * t,
		// accelerating from zero velocity
		...
		// acceleration until halfway, then deceleration
		easeInOutQuint: (t) => t < 0.5 ? 16 * t * t * t * t * t : 1 + 16 *
			--t * t * t * t * t,
	},
	/**
	...
	getJSON: (url, callback) => {
		node();
		fetch(url).then((res) => res.json()).then((json) => callback(
			json)).catch((error) => {
	...
	getHTML: (url, callback) => {
		node();
		fetch(url).then((res) => res.text()).then((html) => callback(_$
			.parseHTML(html))).catch((error) => {
			throw new Error(error.stack);
		});
	},
	/**
	...
	hashString: (str, seed = 0) => {
		...
		h1 = Math.imul(h1 ^ (h1 >>> 16), 2246822507) ^ Math.imul(h2 ^ (
			h2 >>> 13), 3266489909);
		h2 = Math.imul(h2 ^ (h2 >>> 16), 2246822507) ^ Math.imul(h1 ^ (
			h1 >>> 13), 3266489909);
	...
	blendColors: (color1, color2, percent = 50) => {
		...
		const mix = (start, end, percent) => start + (percent / 100) * (
			end - start);
	 ...
	 */
	elementSiblings: (n) => [...n.parentElement.children].filter((c) => c !=
		n),
	/**
	...
	sanitizeHTML: (input, tags = undefined, attributes = undefined) => {
		node();
		var tags = tags || ["A", "B", "BODY", "DIV", "BLOCKQUOTE",
			"IMG", "EM", "HR", "I", "H1", "H2", "H3", "H4", "H5",
			"H6", "BR", "ASIDE", "MAIN", "SPAN", "SMALL", "STRONG",
			"UL", "LI", "TABLE", "TH", "TR", "TD", "OL", "P", "S"
		];
		...
		iframe); // necessary so the iframe contains a document
		iframe.contentDocument.body.innerHTML = input;

		function makeSanitizedCopy(node) {
			if (node.nodeType == Node.TEXT_NODE) {
				var newNode = node.cloneNode(true);
			} else if (node.nodeType == Node.ELEMENT_NODE && tags
				.includes(node.tagName)) {
				newNode = iframe.contentDocument.createElement(node
					.tagName);
		...
		var resultElement = makeSanitizedCopy(iframe.contentDocument
			.body);
	...
	inlineCSS: (el) => {
		...
		for (let key in s) {
			let prop = key.replace(/\-([a-z])/g, v => v[1]
			.toUpperCase());
			el.style[prop] = s[key];
		}
	},
	...
	cookies: {
		...
		getItem: (name) => {
			...
			for (var i = 0; i < ca.length; i++) {
				...
				if (c.indexOf(nameEQ) == 0) return c.substring(nameEQ
					.length, c.length);
			}
			return null;
		},
