...
let _temp = {
   ...
   */
  async: fn => {
    const worker = new Worker(URL.createObjectURL(new Blob(["postMessage((".concat(fn, ")());")]), {
  ...
  jsonToCsv: function jsonToCsv(arr, columns) {
    ...
    return [columns.join(delimiter), ...arr.map(obj => columns.reduce((acc, key) => "".concat(acc).concat(!acc.length ? "" : delimiter, "\"").concat(!obj[key] ? "" : obj[key], "\""), ""))].join("\n");
  },
   ...
   */
  mapObjectKeys: (obj, fn) => Array.isArray(obj) ? obj.map(val => _$.mapObjectKeys(val, fn)) : typeof obj === "object" ? Object.keys(obj).reduce((acc, current) => {
    const key = fn(current);
    const val = obj[current];
    acc[key] = val !== null && typeof val === "object" ? _$.mapObjectKeys(val, fn) : val;
    return acc;
  }, {}) : obj,
   ...
   */
  arrayToCSV: function arrayToCSV(arr) {
    let delimiter = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : ",";
    return arr.map(v => v.map(x => isNaN(x) ? "\"".concat(x.replace(/"/g, '""'), "\"") : x).join(delimiter)).join("\n");
  },
  ...
  serializeForm: form => {
    node();
    return Array.from(new FormData(form), field => field.map(encodeURIComponent).join("=")).join("&");
  },
  ...
  formToObject: form => {
    node();
    return Array.from(new FormData(form)).reduce((acc, _ref3) => {
      let [key, value] = _ref3;
   ...
   */
  uuid: function uuid() {
    let seed = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : Math.random(); //Magic. Do not touch.
   ...
   */
  unescapeHTML: str => str.replace(/&amp;|&lt;|&gt;|&#39;|&quot;/g, tag => ({
    "&amp;": "&",
    "&lt;": "<",
    "&gt;": ">",
    "&#39;": "'",
  ...
  replaceText: (el, callback) => {
    ...
    for (var e, t = function () {
      for (var e, t = el, o = [], a = 0; a < t.length; a++) e = t[a].childNodes[0], t[a].hasChildNodes() && 3 == e.nodeType && o.push(e);

      return o;
    }(), o = 0, a = t.length; o < a; o++) e = t[o].nodeValue, t[o].nodeValue = callback(e);
   ...
   * @returns {undefined}
   */
  timeFunction: function timeFunction(fn) {
    let name = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "_$ function timer";
    console.time(name);
    fn();
  ...
  */
  ...
  hexToRGB: hex => {
    ...
    if (h.length === 3) h = [...h].map(x => x + x).join("");else if (h.length === 8) alpha = true;
    h = parseInt(h, 16);
  ...
  querySelector: elem => {
    ...
    function loop(element) {
      if (element.getAttribute("id") && document.querySelectorAll("#".concat(element.getAttribute("id"))).length === 1) {
  ...
  removeComments: el => {
    if (typeof el === "object") {
      ...
      el.innerHTML = el.innerHTML.replace(/<!--[\s\S]*?(?:-->)?<!---+>?|<!(?![dD][oO][cC][tT][yY][pP][eE]|\[CDATA\[)[^>]*>?|<[?][^>]*>?/g, "");
      return el;
    } else if (typeof el === "string") {
      return el.replace(/<!--[\s\S]*?(?:-->)?<!---+>?|<!(?![dD][oO][cC][tT][yY][pP][eE]|\[CDATA\[)[^>]*>?|<[?][^>]*>?/g, "");
    }
  },
  ...
  random: function random(min, max) {
    ...
    let seed = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : Math.random();
  ...
  */
  ...
  spliceArrayBuffer: (arr, start, end, endian) => {
    ...
    for (var i = start, value = 0; i != end; i += direction) value = 256 * value + arr[i];
  ...
  parseHTML: function parseHTML(string) {
    let mimeType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "text/html";
   ...
   */
  syntaxHighlight: function syntaxHighlight(string) {
    let mode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "html";
    ...
    let highlightel = function highlightel(elmnt, mode) {
      let colors = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {}; // Credit to w3schools for this
      ...
      elmntObj.style.fontFamily = colors.fontFamily || "Consolas,'Courier New', monospace";
   ...
   */
  curryFunction: function curryFunction(fn) {
  ...
  getJSON: (url, callback) => {
    node();
    fetch(url).then(res => res.json()).then(json => callback(json)).catch(error => {
  ...
  getHTML: (url, callback) => {
    node();
    fetch(url).then(res => res.text()).then(html => callback(_$.parseHTML(html))).catch(error => {
      throw new Error(error.stack);
    });
  },
   ...
   */
  blendColors: function blendColors(color1, color2) {
    let percent = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 50;
    ...
    const red1 = parseInt("".concat(color1[1]).concat(color1[2]), 16);
    const green1 = parseInt("".concat(color1[3]).concat(color1[4]), 16);
    const blue1 = parseInt("".concat(color1[5]).concat(color1[6]), 16);
    const red2 = parseInt("".concat(color2[1]).concat(color2[2]), 16);
    const green2 = parseInt("".concat(color2[3]).concat(color2[4]), 16);
    const blue2 = parseInt("".concat(color2[5]).concat(color2[6]), 16);
   ...
   */
  urlQuery: function urlQuery(query) {
  ...
  sanitize: function sanitize(html) {
    let tags = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : undefined;
    let attributes = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined;
    return function (tags, attributes) {
      node();
      ...
      var tags = tags || ["I", "P", "B", "BODY", "HTML", "DEL", "INS", "STRONG", "SMALL", "A", "IMG", "CITE", "FIGCAPTION", "ASIDE", "ARTICLE", "SUMMARY", "DETAILS", "NAV", "TD", "TH", "TABLE", "THEAD", "TBODY", "NAV", "SPAN", "BR", "CODE", "PRE", "BLOCKQUOTE", "EM", "HR", "H1", "H2", "H3", "H4", "H5", "H6", "DIV", "MAIN", "HEADER", "FOOTER", "SELECT", "COL", "AREA", "ADDRESS", "ABBR", "BDI", "BDO"];
      ...
      function attribute_matches(element, attribute) {
        let output = attributes.filter(attr => {
          let returnval = attr.attribute === attribute && (attr.tags === "*" || attr.tags.includes(element.tagName)) && attr.regex.test(element.getAttribute(attribute));
          return returnval;
        });
   ...
   * @returns {undefined}
   */
  saveBlob: function saveBlob(blob) {
    let fileName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "output.txt";
    node();
  ...
  markdownToHTML: src => {
    ...
    replace(rx_link, function (all, p1, p2, p3, p4, p5, p6) {
      stash[--si] = p6 ? p6 : p2 ? p4 ? '<img src="' + p4 + '" alt="' + p3 + '"/>' : p1 : '<a href="' + p4 + '">' + unesc(highlight(p3)) + "</a>";
      return si + "\uf8ff";
    }); // table
   ...
   */
  // prettier-ignore
  animate: function animate(start, end, duration, callback) {
    let interval = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 20;
    let num = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : num => num;
    var value = start;
  ...
  memoize: fn => {
    ...
    const cached = function cached(val) {
   ...
   */
  isAsync: val => Object.prototype.toString.call(val) === "[object AsyncFunction]",
  ...
  context: () => {
    ...
    for (let i = 0; i < elements.length; i++) {
      window.addEventListener("contextmenu", e => {
        ...
        e.preventDefault();
        let items = document.querySelectorAll("#".concat(e.target.closest("[contextmenu]").getAttribute("contextmenu"), " menuitem"));
        menu.innerHTML = "";

        for (let j = 0; j < items.length; j++) {
          const contextMenu = items[j];
          menu.innerHTML += "<li onclick=\"".concat(contextMenu.getAttribute("onclick"), "\">").concat(contextMenu.getAttribute("label"), "</li>");
        }
  ...
  cookies: {
    ...
    setItem: function setItem(name, value) {
      let days = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1000;
      node();
      var expires = "";
...
_temp = _temp.sortObj(_temp); // Imports and exports
