...
let _temp = {
  ...
  async: (fn) => {
    const worker = new Worker(
      URL.createObjectURL(new Blob(["postMessage((".concat(fn, ")());")]), {
  ...
  arrayToCSV: function arrayToCSV(arr) {
    ...
    return arr
      .map((v) =>
        v
          .map((x) => (isNaN(x) ? '"'.concat(x.replace(/"/g, '""'), '"') : x))
          .join(delimiter),
      )
      .join("\n")
  },
  ...
  uuid: function uuid() {
    let seed =
      arguments.length > 0 && arguments[0] !== undefined
        ? arguments[0]
        : Math.random() //Magic. Do not touch.
  ...
  querySelector: (elem) => {
    ...
    function loop(element) {
      if (
        ...
        document.querySelectorAll("#".concat(element.getAttribute("id")))
          .length === 1
      ) {
  ...
  */
  ...
  syntaxHighlight: function syntaxHighlight(string) {
    ...
    let highlightel = function highlightel(elmnt, mode) {
      let colors =
        arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {} // Credit to w3schools for this
    ...
    for (let i = 0; i < elements.length; i++) {
      window.addEventListener("contextmenu", (e) => {
        ...
        let items = document.querySelectorAll(
          "#".concat(
            e.target.closest("[contextmenu]").getAttribute("contextmenu"),
            " menuitem",
          ),
        )
        menu.innerHTML = ""
        ...
        for (let j = 0; j < items.length; j++) {
          ...
          menu.innerHTML += '<li onclick="'
            .concat(contextMenu.getAttribute("onclick"), '">')
            .concat(contextMenu.getAttribute("label"), "</li>")
        }
